...
- Agent-driven non-recorded mode `_run_subprocess` at `agent_driven.py:88-131` bypasses session backend; its only explicit exception catches are `TimeoutExpired` → `AgentOutcome(exit_code=None, metadata.timed_out=True)` and `FileNotFoundError` → `AgentOutcome(exit_code=127)`. These catches exist only in the non-recorded subprocess path; recorded mode at `56-85` uses `TerminalSession` without them — see `extension-points.md`.
