...
- `CodexCliAgentRunner(command=["codex","exec"], timeout_seconds=180, prompt_mode="stdin", cwd?, env={}, record_terminal=True)`:
  ...
  - if `record_terminal`: `_run_recorded()` — directly constructs `TerminalSession(...)` at `agent_driven.py:63-70` (not via `session_backend.create_session`), bypassing configured `config.session_backend`. Builds command, if `prompt_mode == "arg"` appends prompt, if `"stdin"` redirects stdin from the existing `agent_prompt.md` via `sh -lc "... < prompt_path"`; `AgentDrivenRunner.run()` wrote that file before invoking the runner. Enters that `TerminalSession` with operator command, `wait_for_exit(timeout_seconds)`, `parse_agent_output(raw_output)`.
...
- `AgentDrivenRunner(agent_runner).run(recipe, run_dir)` → orchestrates: `build_agent_prompt(recipe)` returns prompt text; `run()` writes `agent_prompt.md`, calls agent_runner, `_write_agent_files()` writes `agent_transcript.md` + `agent_outcome.json`, `_screen_from_agent_cast()` replays cast or creates one via `CastRecorder`, returns steps/assertions/raw/screen.
- `build_agent_prompt(recipe)` — returns formatted checks, target command via shlex.quote, recipe context JSON, and instruction to return JSON schema `{"assertions":...,"transcript":...,"notes":...}`; it does not write a file.
