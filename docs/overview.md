...
TUI Verifier is an evidence-first verification harness for terminal and TUI applications. Scripted PTY and process runs record a real terminal session as an **asciinema v2 cast** (`session.cast`) via `TerminalSession`. Non-recorded agent mode (`record_terminal=False`) runs the operator via `subprocess` (bypassing `config.session_backend`) and synthesizes a cast from transcript via `CastRecorder` rather than recording a real terminal; recorded agent mode directly constructs `TerminalSession` and also bypasses the configured session backend. For runs that have a cast, terminal-derived artifacts are produced from it: final text/SVG are reconstructed via `replay_cast()` (pyte) from the cast, while video is rendered by `render_mp4()` calling `agg` directly on the cast then `ffmpeg`. `steps/` renders every `StepResult` (PTY, process, agent synthetic) from stored screen snapshots, not cast replay. `result.json`/`report.md` include independently evaluated assertions (scripted modes), derived `AssertionResult`s from agent check values, synthetic operator step/exit status, and agent artifact paths represented by `RunResult`; the full `AgentOutcome` is serialized only in `agent_outcome.json`. Agent metadata files (`agent_prompt.md`, `agent_transcript.md`, `agent_outcome.json`) are written separately and do not derive from the cast.
...
6. Evidence for terminal-recorded runs is rendered from the cast (final text/SVG/MP4 when applicable); non-recorded agent runs synthesize cast from transcript. `steps/` renders every `StepResult` (PTY, process, agent synthetic) from stored screens, not PTY-only. Scored and written as derived `AssertionResult`s (including agent-reported check values), synthetic operator step/exit status, and paths to agent artifacts as represented by `RunResult` in `result.json` / `report.md`; the full `AgentOutcome` payload is serialized only in `agent_outcome.json`, then aggregated into `latest-report.md`.
