...
- [Testing, CI, Release](testing-ci-release.md) — 31 tests/8 files: video backend tests verify registry/plumbing only (`test_video_backend_roundtrip` resolves only, `test_runner_run_uses_video_backend` sets `render_video=False`; no actual backend invocation/artifact coverage), asciinema/real process requirements, packaging, E2E verification, GitHub Actions workflows (CI attempts sticky PR comment with `continue-on-error`, not guaranteed; triggers PR/main; Release triggers tag/manual), versioning contract, downstream CI usage
- [Plugin Authoring](plugin-authoring.md) — runnable examples: `Path` import, frozen Recipe via `dataclasses.replace` for agent-driven, `create_session()` return-value context-manager requirement (not backend itself), delegating video backend that writes valid MP4 via `render_mp4` (agg → ffmpeg) or explicit NotImplementedError skeleton marked not-selectable, registry limits and agg gate, recipe-level patterns
