...
``0da8f5f`` (the declared base ``refactor/dedupe-report-helpers-drop-dead-defaults``,
PR #89), taken BEFORE the #93 serialization/schema consolidation. It is used
exclusively as the legacy oracle for differential compatibility tests: any
recipe the legacy validator accepted must be accepted by the consolidated
validator with equivalent warnings/errors.

Do not edit this file to "fix" behaviour — it intentionally encodes the
historical contract. If the canonical schema legitimately becomes stricter in
the future, remove the affected cases from the differential corpus with an
...
from termproof.models import RECIPE_VERSION
