...
``conformance/corpus/assertion_cases.json`` and writes each case's observed
``name``/``passed``/``detail`` to ``conformance/corpus/assertions.expected.json``.
...
`spec/OBSERVATION-LOG.md` §4 constraint that makes an absolute path portable.
"""
...
_REPO = os.environ.get("TERMPROOF_PYTHON_REPO") or str(
    Path(__file__).resolve().parents[1] / "python"
)
if not Path(_REPO).is_dir():
...
def main() -> int:
    ...
    document = {
        "_comment": (
            "Generated by conformance/probe_assertions.py against the Python "
